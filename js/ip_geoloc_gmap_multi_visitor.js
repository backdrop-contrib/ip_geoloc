(function ($) {
  ...
  Backdrop.behaviors.addGMapMultiVisitor = {
    attach: function (context, settings) {

      var mapOptions = settings.ip_geoloc_multi_location_map_options;
      if (!mapOptions) {
        alert(Backdrop.t('Syntax error in visitor map options.'));
      ...
      for (key in locations) {
        ...
        if (++i == 1) {
          ...
          mouseOverText = Backdrop.t('Latest Visitor');
        }
        else {
          mouseOverText = Backdrop.t('Visitor #@i', { '@i': i });
        }
        marker = new google.maps.Marker({ map: map, position: position, title: mouseOverText });

        balloonTexts['LL' + position] = Backdrop.t('Lat/long @lat/@lng', {
